...
class SeamCarve:
    ...
    def find_least_important_seam(self, vals) -> list[int]:
        ...
        TODO: Fill in this method
        '''
        self.dirs = [[0 for j in range(self.image_width)] for i in range(self.image_height)]
        self.costs = [[0 for j in range(self.image_width)] for i in range(self.image_height)]

        for j in range(self.image_width):
            self.costs[self.image_height - 1][j] = vals[self.image_height - 1][j]
        for i in range(self.image_height - 2, -1, -1):
            for j in range(self.image_width):
                if j == 0:
                    min_val, dir_val = min(((self.costs[i + 1][j], 0), (self.costs[i + 1][j+1], 1)), key=lambda x: x[0])
                elif j == self.image_width - 1:
                    min_val, dir_val = min(((self.costs[i + 1][j - 1], -1), (self.costs[i + 1][j], 0)), key=lambda x: x[0])
                else:
                    min_val, dir_val = min([(self.costs[i + 1][j - 1], -1), (self.costs[i + 1][j], 0), (self.costs[i + 1][j + 1], 1)], key=lambda x: x[0])
                
                self.costs[i][j] = vals[i][j] + min_val
                self.dirs[i][j] = dir_val
        
        seam = [self.argmin(self.costs[0])]
        for row in range(1, self.image_height):
            col = seam[-1]
            next_col = col + self.dirs[row-1][col]  
            seam.append(next_col)
